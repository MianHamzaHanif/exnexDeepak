...
function setUpHorizontalHeader() {
  // Check if SimpleBar is available before using it
  const simplebarContent = $(".simplebar-content");
  if (simplebarContent.length === 0) {
    console.log('SimpleBar not loaded, skipping horizontal nav setup');
    return;
  }
  
  navbarSize = navBar.width();
  containerWidth = simplebarContent.width();
