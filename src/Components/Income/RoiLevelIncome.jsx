...
const RoiLevelIncome = () => {
  ...
  const fetchRoiLevelIncomeHistory = async () => {
    ...
    try {
      ...
      const lengthRaw = await contract.methods
        .getRoiLevelIncomeHistoryLength(web3State.account)
        .call();
      ...
      for (let i = 0; i < length; i += 1) {
        const row = await contract.methods
          .userRoiLevelIncomeHistory(web3State.account, i)
          .call();

        const fromUser = Array.isArray(row) ? row[0] : row?.fromUser;
        const depositIndexRaw = Array.isArray(row) ? row[1] : row?.depositIndex;
        const levelRaw = Array.isArray(row) ? row[2] : row?.level;
        const amountRaw = Array.isArray(row) ? row[3] : row?.amount;
        const paidAtRaw = Array.isArray(row) ? row[4] : row?.paidAt;
      ...
      console.error("Failed to fetch ROI level income history:", error);
      toast.error("Failed to fetch ROI level income history");
      setHistory([]);
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="app-wrapper RoiLevelIncomePage">
      <Sidebar />
              ...
              {!web3State.isConnected && (
                <div className="col-12">
                  <div className="alert alert-warning d-flex justify-content-between align-items-center">
                    <span>Please connect your wallet to view ROI level income.</span>
                    <button
                      className="btn btn-sm btn-primary"
