...
const LevelIncome = () => {
  ...
  const fetchLevelIncomeHistory = async () => {
    ...
    try {
      ...
      const lengthRaw = await contract.methods
        .getLevelIncomeHistoryLength(web3State.account)
        .call();
      ...
      for (let i = 0; i < length; i += 1) {
        const row = await contract.methods
          .userLevelIncomeHistory(web3State.account, i)
          .call();
        ...
        const paidAtRaw = Array.isArray(row) ? row[3] : row?.paidAt;

        rows.push({
          fromUser: fromUser || "N/A",
          level: Number(levelRaw || 0),
          income: amountRaw ? web3.utils.fromWei(amountRaw.toString(), "ether") : "0",
          date:
            Number(paidAtRaw) > 0
      ...
      console.error("Failed to fetch level income history:", error);
  ...
  return (
    <div className="app-wrapper LevelIncomePage">
      <Sidebar />
                ...
                <div className="col-12">
                  <div className="alert alert-warning d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <span>Please connect your wallet to view level income.</span>
                    </div>
                    <button
                      className="btn btn-sm btn-primary"
                          ...
                          {isLoading ? (
                            ...
                            <tr>
                              <td colSpan="5" className="text-center py-4 bg-transparent">
                                ...
                                <div>Please connect your wallet to view level income</div>
                              </td>
                            </tr>
                          ) : history.length === 0 ? (
                            <tr>
