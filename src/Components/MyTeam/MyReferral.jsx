...
const MyReferral = () => {
  ...
  const getLevelUserAddress = async (contract, walletAddress, level, index) => {
    try {
      // Preferred as per your contract flow
      return await contract.methods.levelUsers(walletAddress, level, index).call();
    } catch (levelUsersError) {
      // Fallback for alternate ABI naming
      return await contract.methods.getLevelUserAt(walletAddress, level, index).call();
    }
  };
    ...
    try {
      ...
      for (const level of levelsToFetch) {
        ...
        for (let i = 0; i < count; i += 1) {
          ...
          const referrerAddress = Array.isArray(userData)
            ? userData[3]
            : userData?.referrer;
          ...
          rows.push({
            ...
            referrer: referrerAddress || "N/A",
            registrationDate,
            level,
            isActive: isRegistered,
            status: isRegistered ? "Active" : "Inactive",
            package: packageAmount,
            activationDate: Number(packageAmount) > 0 ? registrationDate : "N/A",
          });
        }
      }
  ...
  return (
    <div className="app-wrapper MyReferralPage">
      <Sidebar />
                ...
                <div className="col-12">
                  <div className="alert alert-warning d-flex justify-content-between align-items-center">
                    <div>
                      ...
                      <span>Please connect your wallet to view referrals.</span>
                    </div>
                    <button
                      className="btn btn-sm btn-primary"
                          ...
                          {isLoadingReferrals ? (
                            ...
                            <tr>
                              <td colSpan="7" className="text-center py-4 bg-transparent">
                                ...
                                <div>Please connect your wallet to view referrals</div>
                              </td>
                            </tr>
                          ) : filtered.length === 0 ? (
                            <tr>
